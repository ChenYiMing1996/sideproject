...
public class Image_in_Right : MonoBehaviour
{
    ...
    public void go()
    {
        ...
        distance = Mov_Distance * image_scale_x;//求得並儲存距離值(無正負方向)
        time = distance / speed;//求得時間
        image.transform.Translate(Mov_Distance * image_scale_x, 0, 0);//移出畫面待機進入視線
        move_void();
        //      if (time < 2 || time > 0)
        //      {
